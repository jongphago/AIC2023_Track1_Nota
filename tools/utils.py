...
def write_vids(
    trackers, imgs, src_handlers, latency, pose, colors, mc_tracker, cur_frame=0
):
...
def update_result_lists(trackers, result_lists, frame_id):
    ...
    for tracker, result_list in zip(trackers, result_lists):
        outputs = [
            t.tlwh.tolist() + [t.global_id, t.score]
            for t in tracker.tracked_stracks
            if t.global_id < 0
        ]
        for out in outputs:
...
def visualize_det(dets, img, latency, colors, cur_frame):
    ...
    if len(dets) == 0:
        ...
        cv2.putText(
            img, text, (0, txt_size[1]), font, 0.4 * m, (255, 255, 255), thickness=1 * m
        )
        return img
...
def write_map(trackers, img, writer, colors, mc_tracker, cur_frame=0):
    ...
    for cam_id, tracker in enumerate(trackers):
        for track in tracker.tracked_stracks:
            ...
            img = visualize_map(
                loc, img, cam_id, track_id, colors, len_feats, cur_frame
            )
    writer.write(img)
    return origin_img
...
def visualize_map(loc, img, cam_id, track_id, colors, len_feats, cur_frame):
    ...
    cv2.putText(
        img, text, (loc[0], loc[1] + txt_size[1]), font, 0.4 * m, color, thickness=1 * m
    )
